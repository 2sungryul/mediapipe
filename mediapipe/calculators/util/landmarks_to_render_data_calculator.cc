...
absl::Status LandmarksToRenderDataCalculator::Process(CalculatorContext* cc) {
  ...
  if (cc->Inputs().HasTag(kNormLandmarksTag)) {
    ...
    else if (!firstFingerIsOpen && secondFingerIsOpen && thirdFingerIsOpen && fourthFingerIsOpen && isThumbNearFirstFinger(landmarks.landmark(4), landmarks.landmark(8)))
    {
        hand_gesture = "OK";
    }
    else
    {
        hand_gesture = "Nothing";        
    }
    LOG(INFO) <<landmarks.landmark_size()<<','<< hand_gesture <<','<< "State:" << thumbIsOpen << firstFingerIsOpen << secondFingerIsOpen << thirdFingerIsOpen << fourthFingerIsOpen;       
    
    auto* label_annotation = render_data->add_render_annotations();
    auto* text = label_annotation->mutable_text();
    text->set_display_text("Result:"+hand_gesture);
    ...
    label_annotation->mutable_color()->set_b(255);
