...
absl::Status RunMPPGraph() {
  ...
  //added
  const char* gst =  "nvarguscamerasrc sensor-id=0 ! video/x-raw(memory:NVMM), width=(int)1280, height=(int)720, format=(string)NV12 ! \
  ...
  const bool save_video = true;   //added
  if (!save_video) {
      ...
      if (!writer.isOpened()) {
       /* LOG(INFO) << "Prepare video writer.";
        writer.open(absl::GetFlag(FLAGS_output_video_path),
                    ...
                    */
        //const char* dst = "appsrc ! videoconvert ! video/x-raw, format=BGRx ! nvvidconv ! nvv4l2h264enc insert-sps-pps=true ! h264parse ! \
			  //            rtph264pay pt=96 ! udpsink host=203.234.58.121 port=8001 sync=false";
        const char* dst = "appsrc ! videoconvert ! video/x-raw, format=BGRx ! nvvidconv ! nvv4l2h264enc insert-sps-pps=true ! h264parse ! \
			              rtph264pay pt=96 ! udpsink host=125.139.250.214 port=8001 sync=false";
        //const char* dst = "appsrc ! videoconvert ! video/x-raw, format=BGRx ! nvvidconv ! nvv4l2h264enc insert-sps-pps=true ! h264parse ! \
			  //            rtph264pay pt=96 ! udpsink host=172.30.1.71 port=8001 sync=false";
        //const char* dst = "appsrc ! videoconvert ! video/x-raw, format=BGRx ! nvvidconv ! nvv4l2h264enc insert-sps-pps=true ! h264parse ! \
			  //            rtph264pay pt=96 ! udpsink host=203.234.58.153 port=8001 sync=false";
